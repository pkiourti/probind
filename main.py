import numpy as np
import os
from data import DNASeqGenerator


def data_files():
    i = 1
    if os.path.exists(os.path.join('data', 'x_forward_' + str(i))):
        i += 1
...
if __name__ == '__main__':
    ...
    seq = np.asarray([seq_generator.create_random_seq() for _ in range(100)])
    forward = np.asarray(seq)[:, 0]
    reverse = np.asarray(seq)[:, 1]
    binding_value = [seq_generator.create_random_bind_value() for _ in range(100)]
